...
export function installHybridEngineBridgeRoutes({
  ...
  app.post('/api/funnel/intake', signedRawJson, async (req, res) => {
    ...
    const targetWorker = body.targetWorker ? String(body.targetWorker) : 'windows-laptop-worker-01';
  ...
  app.post('/api/worker/heartbeat', signedRawJson, async (req, res) => {
    ...
    const body = packet.body;
    const workerId = String(body.workerId || body.deviceId || req.get('x-asiod-device') || 'windows-laptop-worker-01');
